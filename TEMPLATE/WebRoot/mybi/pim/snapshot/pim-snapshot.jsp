...
<script type="text/javascript">
	...
	function loadFileArea() {
		$('.folderArea').empty();
		$.post('${ctx}/pim_snapshot!getSnapshotList.action', {"snapshot.queryid": v_winParam.queryid}, function(data){
			var jsondata = JSON.parse(data);
			 $(jsondata).each(function(idx, obj) {
				 ...
				 $(d_li).append($('<div><img class="addFileBtn" src="${ctx}/mybi/pim/themes/default/images/query-'+v_type+'.png"></div>'));
	 ...
	 function sbt(){
		 ...
		 if(!v_name){
			 $.messager.alert('<s:text name="common_msg_info"/>','请填写需要保存的快照名称','info'); //操作失败
			 return false;
		 }
		...
		var queryId = v_winParam.queryid;
		var queryUrl = v_winParam.url;
		//设置原有查询参数
	    var param = $.extend({}, opts.queryParams,{
				...
				"sort": opts.sort,
				"order": opts.order,
				"exportColumnList" : JSON.stringify(parent.$('.myui-datagrid2').datagrid2('getVisableColumn2Json')),
		...
		$.extend(param, snap_param);
		 $.post('${ctx}/' + queryUrl, param, function(data){
			 var jsondata = JSON.parse(data);
			if(jsondata.result == 'succ'){
