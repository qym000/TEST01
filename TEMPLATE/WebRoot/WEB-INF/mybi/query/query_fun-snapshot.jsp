...
<script type="text/javascript">
	...
	function loadFileArea() {
		$('.folderArea').empty();
		$.post('${ctx}/query_online!getSnapshotList.action', {"query_snapshot.queryid":parent.window.getUrlParam("id")}, function(data){
			var jsondata = JSON.parse(data);
			 $(jsondata).each(function(idx, obj) {
				 var jsonobjparam1 = JSON.parse(obj.param1);
				 var d_li = $('<li></li>').appendTo($('.folderArea'));
				 $(d_li).attr('id', obj.id);
				 $(d_li).attr('name', obj.name);
				 $(d_li).attr('filetype', jsonobjparam1.exportFileType);
				 $(d_li).attr('filename', obj.filler1);
				 var v_type = '';
				 if(jsonobjparam1.exportFileType == 'txt') {
					 v_type = 'txt';
				 } else {
					 v_type = 'excel';
				 }
				 $(d_li).append($('<div><img class="addFileBtn" src="${ctx}/mybi/query/themes/default/images/query-'+v_type+'.png"></div>'));
				 $(d_li).append($('<div class="filename">'+jsonobjparam1.exportFileName +'</div>'));
				 if(obj.filler2 == 2) {
					 $(d_li).remove();
				 } else if (obj.filler2 == 0 || obj.filler2 == 1) {
					 $(d_li).addClass('folderAreaRun');
					 var dv_del = $('<div class="runsnap">正在加工..</div>').appendTo($(d_li));
				 } else {
					 $(d_li).find('.addFileBtn').parent().click(function() {
							window.location.href= "${ctx}/query_online!snapshotFileDownload.action?exportFileName=" + $(this).parent().attr('name') + "&exportFileType=" + $(this).parent().attr('filetype') + "&query_snapshot.filler1=" + $(this).parent().attr('filename');
						 })
					 ...
					 $(d_li).mouseenter(function(e){
		        			...
		        			$(dv_del).click(function(){
		        				...
		        				var v_delparam = {
		    							"query_snapshot.id" :  $(this).parent().attr('id'),
		    							"query_snapshot.filler1" : $(this).parent().attr('filename')
		    					}
		        				$.post('${ctx}/query_online!deleteQuerySnapshot.action', v_delparam, function(data){
		        					var jsondata = JSON.parse(data);
	 ...
	 function sbt(){
		...
		var snap_param = {
				"query_snapshot.queryid" : queryId,
				"query_snapshot.type" : '1',
				"query_snapshot.name" : v_name,
				"query_snapshot.param1" : JSON.stringify(param)
		}
		$.extend(param, snap_param);
		 $.post('${ctx}/query_online!remoteQueryExport.action', param, function(data){
			 var jsondata = JSON.parse(data);
			if(jsondata.result == 'succ'){
