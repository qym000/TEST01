...
<script>
	...
	function del(type){
		var v_param = {
			delType         : type,
			"query_dataset.id" : v_winParam.id,
			"query_dataset.pid" : v_winParam.pId
		}
		
		$.post("${ctx}/query_dataset!deleteDatasetFolder.action", v_param,function(data){
			
			var jsondata = JSON.parse(data);
			if(jsondata.result == 'succ'){
				$.messager.alert('<s:text name="common_msg_info"/>','删除'+v_winParam.name+'成功','info', function(){
					parent.parent.parent.window.refreshNodeById(v_winParam.pId);
					parent.parent.window.refreshNodeById(v_winParam.pId);
					parent.parent.window.initDatasetFrame();
					clsWin();
				});
