...
<script type="text/javascript">
	$(function(){
		...
		$("#isdevuse2").combo({
			...
			defaultValue : '${request.obj.isdevuse}'
		});
		...
		$.formValidator.initConfig({formID:"form_input",
			...
			onSuccess:function(){
				...
				param+="&actionObj.id="+$.trim($("#id").val());
				...
				param+="&isdevuse="+isdevuse;
				add_onload();//开启蒙板层
				$.post("pim_sys-menu!updateSysActionObj.action",param,function(data){ 
					if(data.result=="succ"){
						parent.findDataBySysMenuId('${sysMenu.id}');	//回显刚才操作的记录
						$.messager.alert('<s:text name="common_msg_info"/>','<s:text name="common_msg_updatesucc"/>','info',function(){
							clsWin();
						});
		...
		$("#code").formValidator({onFocus:''}).inputValidator({min:1,onError:'<s:text name="sysauth_sysAction_code"/><s:text name="common_msg_formvalidte_required"/>'})
			.ajaxValidator({
				dataType : "text",
				url : "pim_sys-menu!isSysActionCodeUnique.action?code_tmp="+encodeURIComponent('${obj.code}'),
				type : "POST",
				data: {code:$("#code").val()},
		...
		<form id="form_input" method="post">
			 ...
			 <div class="item">
				...
				<ul>
					...
					<li><input id="code" name="code" maxlength="50" value="${request.obj.code}" class="myui-text" style="width:200px;"/></li>
				</ul>
			 </div>
			 <div class="item">
				<ul>
					<li class="desc"><b>* </b><s:text name="sysauth_sysAction_nam2"/>：</li>
					<li><input id="nam" name="nam" maxlength="20" value="${request.obj.nam}" class="myui-text" style="width:200px;"/></li>
				</ul>
				<ul>
					<li class="desc"><b>* </b><s:text name="sysauth_sysAction_namEg"/>：</li>
					<li><input id="namEg" name="namEg" maxlength="30" value="${request.obj.namEg}" class="myui-text" style="width:200px;"/></li>
				</ul>
			 </div>
			 <div class="item">
				<ul>
					<li class="desc">URL：</li>
					<li><textarea id="url" name="url" class="myui-textarea" style="width:506px;height:50px;">${request.obj.url}</textarea></li>
				</ul>
			 </div>
			 <div class="item">
				<ul>
				...
				<ul>
					...
					<li><input id="ord" name="ord" maxlength="3" value="${request.obj.ord}" class="myui-text" style="width:200px;"/></li>
				</ul>
			 </div>
		 </form>
	</div>
