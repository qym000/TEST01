...
<script type="text/javascript">
    ...
    function findResList(){
    	...
    	var _data="";
    	add_onload();//开启蒙板层
    	$.post("pim_spres-sys-menu!findSpresList.action",{ids_user:ids_user},function(data){ 
    		if(data.objList.length>0){
	    		$.each(data.objList,function(idx,item){
	    			_data+="<tr style='background:#EEEEEE'>";
	    			_data+="<td>";
	    			if(item.lvl>=2){
	    			...
	    			_data+="</td>";
	    			_data+="<td style='background:#FCF9F9'>";
    ...
    function sbt(){
		...
		add_onload();//开启蒙板层
		$.post("pim_spres-sys-menu!assignSpresList.action",{ids_user:ids_user,ids_menu:ids_menu,ids_action:ids_action},function(data){ 
			if(data.result=="succ"){
