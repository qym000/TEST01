...
<script type="text/javascript">
	var mustExcel = ${request.mustExcel}; // 是否必须为Excel文件
	...
	// 截取数据月
	function getPmonth(cendat) {
		return cendat.substring(0,6);
	}
	
	// 重新获取数据日期(对月份选择的获取其最后一天)
	function getRealCendat() {
		if (dateType == "0") {
			return $("#cendat").datebox("getValue");
		}else {
			return $("#cendat").datebox("getLastDay");
		}
	}
	...
	function initPage() {
		...
		if (dateType == "0") {
			$("#cendat").datebox({
				dateFormat : 'YYYYMMDD',
				scrollTarget : $("body",parent.document),
				defaultDate : '${request.paraCendat}'
			});
		}else {
			$("#cendat").datebox({
				calendarType : 'month',
				dateFormat : 'YYYYMM',
				defaultDate : getPmonth("${request.paraCendat}")
			});
		}
		// 表单验证初始化
		$.formValidator.initConfig({formID:"form_input",
			...
			onSuccess:function(){
				...
				var paramObj = {
					"taskObj.cendat" : getRealCendat(), // 数据日期
					"taskObj.schemeId" : "${request.schemeId}", // 方案ID
					schemeId : parent.currentScheme
				}; // 传入Action的参数
				add_onload();// 开启蒙板层
				/*
				$.post("${ctx}/gdp_upload!checkCendatUnique.action",paramObj,function(data){
					...
					if (data.isExist) {
						...
						add_onload();*/
						$.ajaxFileUpload({
							url : "gdp_upload!uploadDataFile.action", // 用于文件上传的服务器端请求地址
		...
		// 方案名称不能为空
		$("#cendat").formValidator().inputValidator({min:1,onError:"<s:text name='gdp_common_cannotBeEmpty'/>"});
	}
	...
	function fileOnChange(target) {
		...
		// 文件类型验证
		if (fileType != "xls" && fileType != "xlsx" && mustExcel) {
			fileTypeValid = false;
		}else {
			fileTypevalid = true;
		}
	...
	<div class="form">
		<form id="form_input" method="post">
			 <div class="item">
				<ul>
					<!-- 数据日期 -->
					<li class="desc"><b>* </b><s:text name="gdp_upload_cendat"/>：</li>
					<li><input id="cendat" name="cendat" style="width:160px;"/></li>
				</ul>
			 </div>
			 <div class="item">
				<ul>
					<!-- 数据文件 -->
					<li class="desc"><b>* </b><s:text name="gdp_upload_dataFile"/>：</li>
					<li>
