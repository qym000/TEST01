...
<script type="text/javascript">
	...
	function initPage() {
		...
		$("#schemeType").combo({
			...
			panelHeight : 110,
			defaultValue : "${schemeObj.schemeType}",
		...
		$("#classId").combo({
			...
			valueField : "classId",
			panelHeight : 150,
			defaultValue : "${schemeObj.classId}"
		});
		// 动作列表下拉初始化
		$("#actcde").combo({
			...
			onLoadSuccess : function() {
				var arr = "${interconfObj.actcde}";
				$(this).combo("setValues",arr.split(","));
			}
		});
		$("#dateType li").removeClass().addClass("option");
		$("#dateType li[val='${schemeObj.dateType}']").removeClass().addClass("option_selected");
		// 表单验证初始化
		$.formValidator.initConfig({formID:"form_input",
			...
			onSuccess:function(){
				// 参数对象
				var paramObj = {
					"schemeObj.schemeId" : "${schemeObj.schemeId}",
				...
				//开启蒙板层
				add_onload();
				$.post("${ctx}/gdp_scheme!updateSchemeObj.action",paramObj,function(data){ 
					//关闭蒙板层
					clean_onload();
					if(data.result=="succ"){
						//回显刚才操作的记录
		...
		<form id="form_input" method="post">
			 <div class="item">
				<ul>
					...
					<li><input id="schemeName" name="schemeName" value="${schemeObj.schemeName}" maxlength="50" class="myui-text" style="width:160px;"/></li>
			 ...
			 <div class="item" id="unitInputDiv" style="display:none;">
				<ul>
					...
					<li><input id="unit" name="unit" value="${schemeObj.unit}" maxlength="10" class="myui-text" style="width:160px;"/></li>
			 ...
			 <div class="item">
				<ul>
					...
					<li><input id="fqName" name="fqName" class="myui-text" maxlength="200" style="width:160px;" value="${interconfObj.fqName}"/></li>
				</ul>
			 </div>
			 <div class="item">
				<ul>
					...
					<li><textarea id="schemeDesc" name="schemeDesc" class="myui-textarea" style="width:160px;">${schemeObj.schemeDesc}</textarea></li>
