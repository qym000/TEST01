...
<script type="text/javascript">
 ...
 // 页面元素初始化
	...
	function fileOnChange(target) {
		...
		var path = $(target).val();
		// 将文件路径赋给文件文本框
		$("#" + $(target).attr("id") + "Text").val(path);
	}
	
	// 编辑Awk脚本文件
	function editAwkFile() {
		// 获取上传的awk文件在本地的路径
		var path = $("#awkScriptText").val();
		// 重新赋值为提示信息"点击选择文件"
		$("#awkScriptText").val("<s:text name='gdp_config_clickToSelectFile'/>");
		if (path == "" || path == "<s:text name='gdp_config_clickToSelectFile'/>") {
			$("#inputWin").window({
				open : true,
				headline:"编辑AWK脚本文件",
				content:'<iframe src=gdp_scheme-erms!toEditAwk.action?editFlag=0&id='+schemeId+' scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
				panelWidth:700,
				panelHeight:550
			});
		}else {
			// ajax上传awk脚本
			$.ajaxFileUpload({
				url : "gdp_scheme-erms!uploadAwkFile.action", // 用于文件上传的服务器端请求地址
				secureuri : false, // 一般设置为false
				data : {id : schemeId},
	            fileElementId : 'awkScript', // 文件上传DOM元素的id属性
	            ...
	            success : function(data, status){ // 服务器成功响应处理函数
	            	...
	            	$("#inputWin").window({
	    				open : true,
	    				headline:"编辑AWK脚本文件",
	    				content:'<iframe src=gdp_scheme-erms!toEditAwk.action?editFlag=1 scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
	    				panelWidth:700,
	    				panelHeight:550
	    			});
	            }
	...
	// 上传ERMS文件
	function uploadErmsFile() {
		// ajax上传erms报表文件
		$.ajaxFileUpload({
			url : "gdp_scheme-erms!uploadErmsFile.action", // 用于文件上传的服务器端请求地址
			secureuri : false, // 一般设置为false
			data : {id : schemeId},
            fileElementId : 'ermsRpt', // 文件上传DOM元素的id属性
            dataType : 'text/json', // 返回值类型 一般设置为json
            success : function(data, status){ // 服务器成功响应处理函数
            	if (data.result == "fail") {
            		// 上传失败提示异常信息;
            		...
            		$.messager.alert('<s:text name="common_msg_info"/>', "上传成功", 'info');
            	}
            }
		});
	}
	...
	// 解析ERMS报表文件转换为CSV文件
	function parseErmsToCsv() {
		$.post("${ctx}/gdp_scheme-erms!parseErmsToCsv.action",{id : schemeId},function(data){
			if (data.result == "succ") {
				// 解析成功
        		$.messager.alert('<s:text name="common_msg_info"/>', "解析成功", 'info', toPreviewCsv);
	...
	// 预览解析后的CSV
	function toPreviewCsv() {
		// 打开转换函数添加窗口
    	$("#inputWin").window({
			open : true,
			headline:"解析预览(只显示100行)",
			content:'<iframe src=gdp_scheme-erms!toPreviewCsv.action scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
			panelWidth:1100,
			panelHeight:550
		});
	}
	...
	function saveConfig() {
		var sp = $("#spliter").val();
		if (sp == "") {
			$.messager.alert('<s:text name="common_msg_info"/>',"分割符不能为空","info");
			return;
		}
		...
		add_onload();//开启蒙板层
		$.post("${ctx}/gdp_scheme-erms!saveSchemeConfig.action",paramObj,function(data){
			clean_onload();
			if (data.result == "succ") {
				// 保存成功提示
	...
	function parseFileColumn() {
		...
		// 解析模板文件参数对象
    	var paramObj = {
    		headLine : $.trim($("#headLine").val()), // 列头起始行
    		quoteChar : $("#quoteChar").combo("getValue"),
    		filterWord : $("#filterWord").val(),
    		spliter : $.trim($("#spliter").val())
    	};
    	// 解析模板文件
    	$.post("gdp_scheme-erms!parseFileColumn.action",paramObj,function(data){
    		if (data.result == "succ") {
    			// 加载文件列头Ztree
	...
	function toAwkHelp() {
		$("#inputWin").window({
			open : true,
			headline:"帮助",
			content:'<iframe src=gdp_scheme-erms!toAwkHelp.action scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
			panelWidth:1100,
			panelHeight:550
		});
	}
...
<div class="myui-layout" style="margin-left:15px;">
	<div class="rowgroup">
		<div class="content" style="width:400px;height:180px;" title="不规则文本解析">
			<div class="operate">
            	<ul>
            		<!-- 帮助 -->
                	<li><a href="javascript:void(0)"
                    	onclick="toAwkHelp()" style="color:#374fff;">帮助</a></li>
                </ul>
            </div>
			<div style="width:395px;height:110px;">
				<table class="form-table">
					<tr>
						<!-- AWK脚本文件 -->
						<td class="label">AWK脚本文件:</td>
						<td class="input">
							<input id="awkScriptText" class="myui-fileupload-text" type="text" readonly="readonly" style="width:90px;" value="<s:text name='gdp_config_clickToSelectFile'/>"/>
							<span class="myui-fileupload-wrap" style="position:relative;display:inline-block">
								<!-- 浏览 -->
								<a href="javascript:void(0)" class="myui-fileupload-btn" style="position:absolute;z-index:-9"><s:text name="gdp_upload_browse"/></a></a><input type="file" id="awkScript" name="awkScript" onchange="fileOnChange(this)" style="width:43px;height:22px;cursor: pointer;vertical-align:middle;position:absolute;opacity: 0;filter: alpha(opacity=0);"/>
							</span>
							<!-- 上传 -->
							<a href="javascript:void(0)" class="myui-fileupload-btn" onclick="editAwkFile()">上传</a>
						</td>
					</tr>
					<tr>
						<!-- ERMS报表模板 -->
						<td class="label" style="width:120px">不规则文本文件:</td>
						<td class="input">
							<input id="ermsRptText" class="myui-fileupload-text" type="text" readonly="readonly" style="width:90px;" value="<s:text name='gdp_config_clickToSelectFile'/>"/>
							<span class="myui-fileupload-wrap" style="position:relative;display:inline-block">
								<!-- 浏览 -->
								<a href="javascript:void(0)" class="myui-fileupload-btn" style="position:absolute;z-index:-9"><s:text name="gdp_upload_browse"/></a></a><input type="file" id="ermsRpt" name="ermsRpt" onchange="fileOnChange(this)" style="width:43px;height:22px;cursor: pointer;vertical-align:middle;position:absolute;opacity: 0;filter: alpha(opacity=0);"/>
							</span>
							<!-- 上传 -->
							<a href="javascript:void(0)" class="myui-fileupload-btn" onclick="uploadErmsFile()"><s:text name="gdp_common_upload"/></a>
						</td>
					</tr>
				</table>
			</div>
			<div style="border-top:1px solid #EEE;width:395px;height:30px;text-align:right;">
				<a href="javascript:void(0)" onclick="parseErmsToCsv()" class="myui-button-query" style="margin-top:5px;">
					解析</a>
			</div>
		</div>
		<div class="content" style="width:800px;height:180px;" title="方案配置选项">
			<div style="width:795px;height:110px;">
				<table class="form-table">
					<tr>
						<!-- 数据库表 -->
					...
					<tr>
						...
						<td class="input" ><input id="headLine" name="headLine" class="myui-text" style="width:180px;"/></td>
						<td class="split"></td>
						<!-- 引号字符 -->
					...
					<tr id="csvConfDiv">
						<!-- 分隔符 -->
						<td class="label"><b>* </b><s:text name="gdp_config_spliter"/>:</td>
			...
			<div style="border-top:1px solid #EEE;width:795px;height:30px;text-align:right;">
				<!-- 保存 -->
				<a href="javascript:void(0)" onclick="parseFileColumn()" class="myui-button-query" style="margin-top:5px;">
					解析字段</a>
			</div>
		</div>
	</div>
