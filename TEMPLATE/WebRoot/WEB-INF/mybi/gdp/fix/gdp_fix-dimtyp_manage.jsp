...
<script type="text/javascript">
	...
	function initPage() {
		// 回车事件
		$("#dimtypCode").bind("keydown",function(e){
			if (e.keyCode == 13) {
				cx(1);
			}
		});
	...
	function cx(page) {
		// 查询参数;
		var paramObj = {
			"dimtypObj.dimtypCode" : $("#dimtypCode").val(),
			page : page,
		...
		$.post("${ctx}/gdp_fix-dimtyp!findDimtypPager.action",paramObj,function(data){
			...
			if (data.datalist != null && data.datalist.length > 0) {
				$.each(data.datalist, function(idx,item){
					htm += "<tr>";
	    			htm += "<td><input type='checkbox' name='checkboxitem' value='"+item.dimtypId+"'></td>";
	    			htm += "<td>"+ item.dimtypCode +"</td>";
	...
	function findListById(id) {
		$.post("${ctx}/gdp_fix-dimtyp!findDimtypById.action",{"dimtypObj.dimtypId" : id},function(data){ 
    		$(".myui-datagrid-pagination").html('');
    		var htm = "";
    		htm += "<tr>";
			htm += "<td><input type='checkbox' name='checkboxitem' value='" + data.dimtypId + "'></td>";
			htm += "<td>" + data.dimtypCode + "</td>";
	...
	function add() {
		$("#inputWin").window({
			...
			content:'<iframe src=gdp_fix-dimtyp!toDimtypInput.action scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
			panelWidth:500,
			panelHeight:200
		});
	}
	...
	function upt() {
    	...
    	$("#inputWin").window({
			...
			content:'<iframe src=gdp_fix-dimtyp!toDimtypUpdate.action?id='+ id +' scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
			panelWidth:500,
			panelHeight:200
		});
	}
	...
	function del() {
    	...
    	add_onload();
    	$.post("${ctx}/gdp_fix-dimtyp!deleteDimtypObj.action",{id : idArr.join(",")},function(data){
     		clean_onload();
    		if(data.result=="succ"){
    			// 操作成功
...
<div class="myui-template-condition">
    <ul>
   	    <li class="desc" style="width:140px">维度类型编码/名称：</li>
        <li>
      	    <input type="text" id="dimtypCode" name="dimtypCode" class="myui-text" style="width:160px" title='<s:text name="common_msg_fuzzy_query"/>'/>
	    </li>
    </ul>
</div>
...
<div class="myui-datagrid">
	<table>
		<tr>
			...
			<th field="dimtypCode" sortable="true">维度类型编码</th>
			<th field="dimtypName" sortable="true">维度类型名称</th>
		</tr>
		<tbody id="databody">
		</tbody>		
	</table>
