			...
			onSuccess:function(){
				...
				$.post("${ctx}/gdp_upload!checkCendatUnique.action",paramObj,function(data){
					...
					if (data.isExist) {
						...
						add_onload();*/
						$.ajaxFileUpload({
							url : "gdp_fix-item!writeToFile.action", // 用于文件上传的服务器端请求地址
				            ...
				            success : function(data, status){ // 服务器成功响应处理函数
				            	...
				            	if (data.result == "succ") {
				            		// 操作成功提示信息;
				            		$.messager.alert('<s:text name="common_msg_info"/>', "已将项目编号回填到文件中，点击确定后下载", 'info',function(){
				            			parent.window.location.href = "${ctx}/gdp_fix-item!downloadItemWritedFile.action";
				            		});
				            	}else {
