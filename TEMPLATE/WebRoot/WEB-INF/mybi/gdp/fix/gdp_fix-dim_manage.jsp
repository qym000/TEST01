...
<script type="text/javascript">
  ...
  function initPage() {
    $("#dimtypId").combo({
    	...
    	customData : [{dimtypId : "", dimtypName : "全部"}]
    });
    // 回车事件
    $("#dimCode").bind("keydown",function(e){
      if (e.keyCode == 13) {
        cx(1);
      }
    });
  ...
  function cx(page) {
    ...
    $.post("${ctx}/gdp_fix-dim!findDimPager.action",paramObj,function(data){
      ...
      if (data.datalist != null && data.datalist.length > 0) {
        $.each(data.datalist, function(idx,item){
        	htm += "<tr>";
            htm += "<td><input type='checkbox' name='checkboxitem' value='"+item.dimId+"'></td>";
            htm += "<td>"+ item.dimCode +"</td>";
            htm += "<td>"+ item.dimInfo +"</td>";
            htm += "<td>"+ item.dimtypName +"</td>";
            htm += "<td>"+ item.dimKeyword + "</td>";
            htm += "</tr>";
        });
      }else {
        // 没有数据
  ...
  function findListById(id) {
    $.post("${ctx}/gdp_fix-dim!findDimById.action",{"dimObj.dimId" : id},function(data){ 
        $(".myui-datagrid-pagination").html('');
        var htm = "";
        htm += "<tr>";
      htm += "<td><input type='checkbox' name='checkboxitem' value='" + data.dimId + "'></td>";
      htm += "<td>"+ data.dimCode +"</td>";
      htm += "<td>"+ data.dimInfo +"</td>";
      htm += "<td>"+ data.dimtypName +"</td>";
      htm += "<td>"+ data.dimKeyword + "</td>";
      htm += "</tr>";
      $("#databody").html(htm);
    },"json");  
  }
  ...
  function add() {
    $("#inputWin").window({
      ...
      content:'<iframe src=gdp_fix-dim!toDimInput.action scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
      panelWidth:500,
      panelHeight:350
    });
  }
  ...
  function upt() {
    var objsChecked = $("input[name='checkboxitem']:checked");
      ...
      content:'<iframe src=gdp_fix-dim!toDimUpdate.action?id='+ id +' scrolling="no" frameborder="0" style="width:100%;height:100%;overflow:hidden;" />',
      panelWidth:500,
      panelHeight:350
    });
  }
  ...
  function del() {
      ...
      add_onload();
      $.post("${ctx}/gdp_fix-dim!deleteDimObj.action",{id : idArr.join(",")},function(data){
        clean_onload();
        if(data.result=="succ"){
          // 操作成功
...
<div class="myui-template-condition">
    <ul>
        <li class="desc" >维度类型：</li>
        <li>
            <input type="text" id="dimtypId" name="dimtypId" class="myui-text" style="width:160px"/>
        </li>
        <li class="desc" style="width:120px">维度编码/名称：</li>
        <li>
            <input type="text" id="dimCode" name="dimCode" class="myui-text" style="width:160px" title='<s:text name="common_msg_fuzzy_query"/>'/>
        </li>
    </ul>
</div>
...
<div class="myui-datagrid">
  <table>
    <tr>
      ...
      <th field="dimCode" sortable="true">维度编码</th>
      <th field="dimInfo" sortable="true">维度名称</th>
      <th field="dimtypId" sortable="true">维度类型</th>
      <th>关键词</th>
    </tr>
    <tbody id="databody">
    </tbody>    
