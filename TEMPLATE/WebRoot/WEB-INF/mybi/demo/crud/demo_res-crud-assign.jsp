...
<script type="text/javascript">
	...
	var setting = {
		...
		async: {
			enable: true,
			url:"demo_res-crud!findResList.action",
    ...
    //提交分配
    function sbt(){
		var resTree=$.fn.zTree.getZTreeObj("resTree");
		var o_nodes = resTree.getCheckedNodes(true);
		
		var ids_crud="";
		for(i=0;i<o_nodes.length;i++){
			ids_crud+=o_nodes[i].id+",";
		}
		
		add_onload();//开启蒙板层
		$.post("demo_res-crud!assignResList.action",{ids_role:ids_role,ids_crud:ids_crud},function(data){ 
			if(data.result=="succ"){
