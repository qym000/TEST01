...
<script type="text/javascript">
	var setting = {
		...
		async: {
			...
			url:"demo_dev-template!getDemoTree.action" // 异步请求url
		},
		callback : {
	...
	function zTreeOnClick(event, treeId, treeNode) {
		add_onload();
		$("#api").attr("src","demo_dev-template!toContent.action?id=" + treeNode.id);
	}
	...
	function ztreeOnAsyncSuccess(event,treeId,treeNode,msg) {
		...
		if (nodes != null && nodes.length > 0) {
			add_onload();
			$("#api").attr("src", "demo_dev-template!toContent.action?id=" + nodes[0].id)
		}
	}
